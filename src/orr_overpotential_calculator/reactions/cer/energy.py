#!/usr/bin/env python3
"""
Calculate energies for CER-related structures
"""
...
MOLECULES = {
    # CER: gas-phase reference
    "Cl2": Atoms("ClCl", positions=[(0, 0, 0), (0, 0, 1.99)]),
    # CER: adsorbate (only Cl is placed on surface; O presence is encoded in the input surface)
...
ADSORBATE_HEIGHT = 0.5  # Default initial height of adsorbate above surface (Å)
...
def calculate_adsorption_on_site(
    ...
    add_adsorbate(slab_with_adsorbate, adsorbate, height=height, position=site)
...
def calculate_adsorption_with_offset(
    ...
    if height is None:
        height = ADSORBATE_HEIGHT
    add_adsorbate(slab_with_adsorbate, adsorbate, height, offset=offset)
