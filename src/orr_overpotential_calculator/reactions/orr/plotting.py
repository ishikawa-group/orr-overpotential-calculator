...
def my_calculator(
    ...
    """Compatibility wrapper over the shared backend selector implementation."""
    from ...common.calculators import my_calculator as shared_my_calculator

    return shared_my_calculator(
        atoms,
        kind=kind,
...
def generate_result_csv(
    ...
    import csv
    from pathlib import Path
    from .overpotential import compute_reaction_energies, \
    ...
    for material_name, json_path in materials_data.items():
        ...
        try:
            deltaEs, energies = compute_reaction_energies(
                results,
                E_slab,
